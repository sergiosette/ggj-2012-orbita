...
namespace Esfera
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game, IAgendadorListener
    {
        ...
        private Texture2D[] tileArray = new Texture2D[10];
        private Texture2D[] nucleoArray = new Texture2D[10];
        private Texture2D[] backgroundArray = new Texture2D[20];
        private Texture2D[] inimigosArray = new Texture2D[10];
        ...
        {   
            ...
            bool acabouGerar = false;
            int j = 0;
            List<int> elementosGerados = new List<int>();

            while (!acabouGerar)
            {
                int numeroGerado = randomGenerator.Next(1, quantidade);
                if (!elementosGerados.Contains(numeroGerado)) elementosGerados.Add(numeroGerado);
                if (elementosGerados.Count >= maxTiles + 1) acabouGerar = true;
            }
            ...
            {
                if (elementosGerados.Contains(i)) tiles.Add(true);
                else tiles.Add(false);
            }
            tiles.Add(false);
            return tiles;
        }
        ...
        protected override void LoadContent()
        {
            ...
            int inimigoCarregado = randomGenerator.Next(1, 6);
            imageInimigo = this.Content.Load<Texture2D>("temas/inimigos/" + inimigoCarregado);

            //fpsCounter = int.Parse(this.config.getFPS());
            Font1 = Content.Load<SpriteFont>(@"Arial");

            imageMainPage = this.Content.Load<Texture2D>("temas/background/start");
        ...
        public void AgendamentoDisparado()
        {
            ...
            {
                int temaRandom = randomGenerator.Next(1,5);
